...
def calculate_distance_haversine(lat1, lon1, lat2, lon2):
    ...
    NB: Taking into account the Earth's curvature isn't that important when dealing with short distances. 
        ...
        To use the Haversine formula, your input must be in latitude and longitude, because the formula is based on the spherical geometry of the Earth.
...
def load_gedi_data(gedi_dir, prefix):#, slope_type):
    ...
    target_crs = "EPSG:6430+5703" # Target CRS: UTM Zone 11N (horizontal) + NAVD88 (vertical) gravity-related height
    ...
    for file in gedi_files:
        ...
        if len(gdf) == 0: #check whether file is empty or not
            ...
            #--------compute slope--------#
            # # if slope_type == 'haversine': # use haversine formular
            # # extract coordinates and elevation for GEDI data
            # latitudes_gedi = gdf["Latitude"][:]
            # longitudes_gedi = gdf["Longitude"][:]
            ...
            if prefix == "GEDI02_A": 
                # col_name2 = prefix+f'{"_slope_haversine"}'
                # # Calculate slope for GEDI data
                # gedi_slopes_haversine = [-999]
                # for i in range(1, len(gdf)):
                #     dist = calculate_distance_haversine(latitudes_gedi.iloc[i-1], longitudes_gedi.iloc[i-1], latitudes_gedi.iloc[i], longitudes_gedi.iloc[i])
                #     delta_elevation = elevations_gedi.iloc[i] - elevations_gedi.iloc[i-1]
                #     slope_haversine = delta_elevation/dist if dist != 0 else 0
                #     gedi_slopes_haversine.append(slope_haversine) 
                # gdf[col_name2] = gedi_slopes_haversine
...
def load_icesat2_data(icesat2_dir, prefix, process = False, filetype = "geojson"):
    ...
    if process == True:
        if prefix == "ATL03": 
            ...
            # computing slope: 
            # method 1: the utm approach


            # # extract coordinates and elevation for ICESat2 data
            # latitudes_icesat2 = icesat2_gdf_atl03_veg_ground["Latitude"][:]
            # longitudes_icesat2 = icesat2_gdf_atl03_veg_ground["Longitude"][:]
            # elevations_icesat2 = icesat2_gdf_atl03_veg_ground["elevation_NAVD88_icesat2_from_crs"][:]
            # # create ICESat-2 GeoDataFrame
            # icesat2_points = [Point(lon, lat) for lon, lat in zip(longitudes_icesat2, latitudes_icesat2)]
            # icesat2_gdf2 = gpd.GeoDataFrame({"elevation_NAVD88_icesat2_from_crs": elevations_icesat2}, geometry=icesat2_points, crs=utm_code)
            # # calculate slope for ICESat-2 data
            # icesat2_slopes_haversine = [-999]
            # for i in range(1, len(icesat2_gdf2)):
            #     dist = calculate_distance_haversine(latitudes_icesat2.iloc[i-1], longitudes_icesat2.iloc[i-1], latitudes_icesat2.iloc[i], longitudes_icesat2.iloc[i])
            #     delta_elevation = elevations_icesat2.iloc[i] - elevations_icesat2.iloc[i-1]
            #     slope_haversine = delta_elevation / dist if dist != 0 else 0
            #     icesat2_slopes_haversine.append(slope_haversine)
            # # icesat2_slopes
            # product = "ICESat2"
            # icesat2_gdf_atl03_veg_ground[product+f'{"_slope_haversine"}'] = icesat2_slopes_haversine

            ###########################################################################
            ###########################################################################
            # Calculate slope for ICESat2 using UTM coordinates. Rise over run
            easting_icesat2 = icesat2_gdf_atl03_veg_ground["Easting"][:]
            northing_icesat2 = icesat2_gdf_atl03_veg_ground["Northing"][:]
            elevations_icesat2 = icesat2_gdf_atl03_veg_ground["elevation_NAVD88_icesat2_from_crs"][:]
            icesat2_slope_ratio_utm = [-999]
            icesat2_slope_degrees_utm = [-999]
            for i in range(1, len(icesat2_gdf_atl03_veg_ground)):
            ...
            icesat2_gdf_atl03_veg_ground[ratio_col_name] = icesat2_slope_ratio_utm
            icesat2_gdf_atl03_veg_ground[degrees_col_name] = icesat2_slope_degrees_utm
            ...
            icesat2_gdf_atl03_veg_ground = calculate_slope_rise_run(icesat2_gdf_atl03_veg_ground, product) # Compute slope using the rise/run approach
            ...
            return icesat2_gdf_atl03, icesat2_gdf_atl03_photons, icesat2_gdf_atl03_veg_ground
        elif prefix == "ATL08":
...
def gdf_to_laz(x_col, y_col, z_col, output_dir, data = None, input_dir = None, load = False):
    ...
    However, you can convert your GeoDataFrame to a LAS format and then compress it to LAZ using the laspy and pyntcloud libraries.

    Steps to Save a GeoPandas DataFrame as a LAZ File:
        1. Convert your GeoDataFrame to a Pandas DataFrame and extract the required x, y, z coordinates.
